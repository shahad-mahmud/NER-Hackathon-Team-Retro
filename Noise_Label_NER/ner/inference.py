...
class ArgsClass:
    def __init__(self):
        ...
        self.model_name_or_path = ["csebuetnlp/banglabert","csebuetnlp/banglabert"]
...
def evaluate(args, model, features, words_list):
    ...
    dataloader = DataLoader(features, batch_size=args.batch_size, shuffle=False, collate_fn=collate_func, drop_last=False)
    preds, keys = [], []
            
    for sen_id,batch in enumerate(dataloader):
        model.eval()
        # print(batch['labels'])
        batch_labels = batch['labels'][int(args.best_model_index)].cpu().numpy().flatten().tolist() 
        
        with torch.no_grad():
            logits = model(input_ids = batch['input_ids'],
                                attention_mask = batch['attention_mask'])
            # logits = model(**batch)[0]
        sen_preds=np.argmax(logits[int(args.best_model_index)][:,:-1].cpu().numpy(), axis=-1).tolist()
...
def main(args):
    ...
    for words,labels in zip(words_list,labels_list):
        ...
        train_features.append(process_instance(words,labels,tokenizers,args, args.max_seq_length,args.is_banner))
    ...
    crit_weights = [0.5, 0.6126589945487584, 0.6241671714112659, 0.6263476680799516, 0.8200484554815264, 0.8202907328891581, 0.6829194427619625, 0.8533615990308905, 0.8241671714112659, 0.842580254391278, 0.5822531798909751, 0.8147183525136281, 0.8307086614173229, 0.9, 0.7407026044821321, 0.7270139309509388, 0.8766202301635373, 0.8762568140520897, 0.8681405208964265, 0.8915202907328892, 0.5, 0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name_or_path", type=str, nargs="+", default=["csebuetnlp/banglabert","csebuetnlp/banglabert"])
    parser.add_argument("--pretrained_path", default="./noisy_output_banner_optim_cleaned/model.pt", type=str)
