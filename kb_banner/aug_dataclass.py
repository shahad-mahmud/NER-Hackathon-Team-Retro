...
class ConllDataset(torch.utils.data.Dataset):
    def __init__(self, name, all_tokens, all_tags):
        self.name = name
        self.sentences = []
        
        for idx,tokens_tags in enumerate(zip(all_tokens,all_tags)):
            tokens,tags = tokens_tags
...
class ConllCorpus(Corpus):
    def __init__(self, name, data_filepath):
        ...
        train = ConllDataset("%s-train" % name, train_texts,train_labels)
        # super(ConllCorpus, self).__init__(train, dev, test, name=name)
        super(ConllCorpus, self).__init__(train, name=name)
