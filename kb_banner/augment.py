...
from aug_dataclass import Sentence, Token
from lexical_db_bangla import bangla_dict
    ...
    for sentence in dataset:
        ...
        for token in sentence:
            ...
            if label == "O" or label[0] == "B":
                ...
                mention = []
            if label[0] == "B": 
                mention.append(label[2:])
            if label != "O":
                if len(mention)==0:
                    mention.append(label[2:])
                    mention.append(token.text)
                else:
                    mention.append(token.text)
    ...
    for i in range(num_generated_samples):
        ...
        for mask, token in zip(masks, sentence.tokens):
            ...
            if mask == 0 or token.text.lower() in STOPWORDS:
                ...
                synonyms = set()
                syns_set=bangla_dict.syns(token.text)
                if syns_set:
                    synonyms = set([word for word in syns_set if word!=token.text])
