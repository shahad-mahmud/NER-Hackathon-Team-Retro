...
def prepare_samples(texts, labels, aug_dict=None):
    sentences, tags = [], []
    ...
    for i, x in enumerate(texts):
        ...
        if aug_dict:
            sentence = ' '.join(x)
            if sentence in aug_dict:
                for s in aug_dict[sentence]:
                    sentences.append(tokens + s.split() + ['[SEP]'])
                    extra_pads = ["<PAD>"] * (len(s.split()) + 1)
                    tags.append(["<PAD>"] + labels[i] + ["<PAD>"] + extra_pads)
                    
                    assert len(sentences[-1]) == len(tags[-1])
        else:
            sentences.append(tokens)
            tags.append(["<PAD>"] + labels[i] + ["<PAD>"])
