...
def train(args, model, train_features, benchmarks):
    train_dataloader = DataLoader(train_features, batch_size=args.batch_size,
                                  shuffle=True, collate_fn=collate_func, drop_last=True)
    ...
    for epoch in range(int(args.num_train_epochs)):
        if epoch > 10:
            ...
            for i in range(num_models):
                parameters.extend([{"params": model.models[i].classifier.parameters(), "lr": 0.0005},
                                   {"params": model.models[i].model.parameters(
                                   ), "lr": 5e-5},
                                   {"params": model.models[i].rnn.parameters(
                                   ), "lr": 0.0005},
                ...
                for tag, features in benchmarks:
                    ...
                    if tag == 'dev':
                        for i in range(args.n_model):
                            val_f1.append(
                                results[f"{tag}_f1_{args.model_name_or_path[i]}_{i}"])
                    wandb.log(results, step=num_steps)

        for i in range(args.n_model):
            model_save_path = os.path.join(
                args.output_dir, f"{args.model_name_or_path[i].split('/')[-1]}_{i}_model.pt")
        ...
        if np.all(patience >= [args.patience]*args.n_model):
            for i in range(args.n_model):
                print(
                    f"No more patience. Existing best model score for {args.model_name_or_path[i]}: {best_val_f1[i]}")
                break
    ...
    best_model_save_path = os.path.join(
        args.output_dir, f"{args.model_name_or_path[best_model_index].split('/')[-1]}_{best_model_index}_model.pt")
    ...
    wandb.run.summary["best_model"] = args.model_name_or_path[best_model_index] + \
        "_"+str(best_model_index)
    ...
    wandb.finish()
    shutil.copyfile(best_model_save_path, os.path.join(
...
def evaluate(args, model, features, epoch=None, stage=None, tag="dev"):
    dataloader = DataLoader(features, batch_size=args.batch_size,
                            shuffle=True, collate_fn=collate_func, drop_last=False)
    ...
    for batch in dataloader:
        ...
        for i in range(args.n_model):
            keys[i] = keys[i] + \
                batch['labels'][i].cpu().numpy().flatten().tolist()
        batch['labels'] = None
        with torch.no_grad():
            ...
            for i in range(args.n_model):
                if args.is_banner:
                    preds[i] = preds[i] + \
                        np.argmax(
                            logits[i][:, :-1].cpu().numpy(), axis=-1).tolist()
                else:
                    preds[i] = preds[i] + \
                        np.argmax(logits[i].cpu().numpy(), axis=-1).tolist()

    if args.is_banner:
        for i in range(args.n_model):
            preds[i], keys[i] = list(zip(
                *[[pred, key] for pred, key in zip(preds[i], keys[i]) if key != len(LABEL_TO_ID)]))
    else:
        for i in range(args.n_model):
            preds[i], keys[i] = list(
                zip(*[[pred, key] for pred, key in zip(preds[i], keys[i]) if key != -1]))
    ...
    with open(os.path.join(args.output_dir, f"{tag}_report_{stage if stage else ''}.txt"), 'a') as f:
        ...
        for i in range(args.n_model):
            ...
            report = classification_report(keys[i], preds[i], target_names=list(
                LABEL_TO_ID.keys()), labels=list(LABEL_TO_ID.values()),  digits=4, zero_division=0)
            f1_macro = f1_score(keys[i], preds[i], labels=list(
                LABEL_TO_ID.values()), average='macro')
            print(report)
            f.write(report)
            output[tag + "_f1"+"_"+args.model_name_or_path[i] +
                   "_"+str(i)] = f1_macro
        return output
...
def main():
    ...
    parser.add_argument("--model_name_or_path", type=str, nargs="+",
                        default=["csebuetnlp/banglabert", "bert-base-multilingual-cased"])
    ...
    if args.is_banner:
        train_features = read_banner(
            data_file, args, is_banner=args.is_banner, datatype='train', max_seq_length=args.max_seq_length)

        dev_features = read_banner(data_file, args, is_banner=args.is_banner,
                                   datatype='val', max_seq_length=args.max_seq_length)

        test_features = read_banner(
            data_file, args, is_banner=args.is_banner, datatype='test', max_seq_length=args.max_seq_length)

    else:
        train_features = read_banner(
            data_file, args, datatype='train', max_seq_length=args.max_seq_length)
        dev_features = read_banner(
            data_file, args, datatype='validation', max_seq_length=args.max_seq_length)
        test_features = read_banner(
            data_file, args, datatype='test', max_seq_length=args.max_seq_length)

    wandb.init(project="banner", entity='shahad001', config=vars(args))
    d = Counter(
        [label for example in train_features for label in example['labels'][0]])
