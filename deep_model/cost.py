...
def crit_weights_gen(a, b, class_no):
    ...
    class_weight = [range_fix(a, b, class_weight[i], max_value, min_value)
                    for i in range(len(class_weight)) if class_weight[i]]
    class_weight = class_weight[:max_example_index] + \
        [a] + class_weight[max_example_index:]
    return class_weight
